// g++ sierpinski_square_openmp.cpp -O3 -fopenmp -o binarka
// ./binarka
...
void task(int l, int iY_vertex, int iX_vertex){
    ...
    */
    #pragma omp task 
    task(l+1, iY_vertex, iX_vertex); //1
    #pragma omp task 
    task(l+1, iY_vertex, iX_vertex + square_size / pow(3, l+1)); //2
    #pragma omp task 
    task(l+1, iY_vertex, iX_vertex + 2 * square_size / pow(3, l+1)); //3
    #pragma omp task 
    task(l+1, iY_vertex + square_size / pow(3, l+1), iX_vertex); //4
    #pragma omp task 
    task(l+1, iY_vertex + square_size / pow(3, l+1), iX_vertex + 2 * square_size / pow(3, l+1)); //5
    #pragma omp task 
    task(l+1, iY_vertex + 2 * square_size / pow(3, l+1), iX_vertex); //6
    #pragma omp task 
    task(l+1, iY_vertex + 2 * square_size / pow(3, l+1), iX_vertex + square_size / pow(3, l+1)); //7
    #pragma omp task 
    task(l+1, iY_vertex + 2 * square_size / pow(3, l+1), iX_vertex + 2 * square_size / pow(3, l+1)); //8
...
int main()
{
    ...
    FILE * fp;
    string filename_str = "odm_lab8_2b_Skyba.ppm";
